/**
 * Deterministic synthetic G1 walker — the frame generator shared by the
 * robot-bridge synthetic provider (Node) and the in-browser demo mode.
 * Emits canonical RobotFrames with plausible gait, IMU bob, foot forces and
 * monotonic battery drain, plus scripted slip / comms-dropout / carried
 * segments. Provenance is always `synthetic` / `SIMULATED`.
 *
 * Deterministic by (seed, tick): the same seed replays the same walk. The
 * caller supplies a parsed route (src/data/robotTransposition.js::parseRoute)
 * so this module stays runtime-agnostic (no filesystem access).
 *
 * @module data/robotSyntheticWalker
 */

import { transpose } from './robotTransposition.js';
...
/**
 * Create the deterministic synthetic walker.
 * @param {{route: {points: Array, lengthM: number}, seed?: number,
 *   id?: string, rateHz?: number, startSAlongM?: number}} options
 * @returns {{nextFrame: (nowMs: number) => object,
 *   progress: () => {sAlongM: number, routeLengthM: number}}}
 */
export function createSyntheticWalker({
  route,
  seed = 5,
  id = 'g1-01',
  rateHz = 10,
  ...
  function progress() {
    return { sAlongM: sAlong, routeLengthM: route.lengthM };
  }

  return { nextFrame, progress };
}

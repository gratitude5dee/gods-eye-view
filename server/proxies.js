/**
 * God's Eye View server middleware — every same-origin `/api/*` route.
 *
 * These are plain Connect-style middlewares wrapped in Vite plugin objects, so
 * one implementation serves three hosts: `vite dev`, `vite preview`, and the
 * serverless dispatcher in `api/gev.js`. Nothing here imports Vite itself.
 *
 * Upstreams proxied (CORS bypass, caching, and server-side key brokering):
 ...
 *  10. NASA FIRMS — live active-fire detections (VIIRS x3, trailing 24 h)
 ...
 *  16. OpenAI Realtime — ephemeral voice-session tokens and HUD summaries
 *  17. Google Places — nearby/text place context for the voice agent
 *
 * @module server/proxies
 */
...
/** Repository root (this module lives in <root>/server). */
const ROOT_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');

/**
 * Resolve a path inside the writable cache tree.
 *
 * Defaults to `<cwd>/.gev-cache`, the long-running-server layout. Serverless
 * runtimes mount the deployment read-only, so GEV_CACHE_DIR (or the Vercel
 * default below) points the same caches at the instance-local temp disk: warm
 * instances still get hits, cold ones simply miss.
 */
function gevCachePath(...parts) {
  const base = process.env.GEV_CACHE_DIR
    || (process.env.VERCEL ? path.join(os.tmpdir(), 'gev-cache') : path.join(process.cwd(), '.gev-cache'));
...
const OVERPASS_DISK_DIR = gevCachePath('overpass');
...
export function rocketLaunchesProxy() {
  ...
  const cachePath = gevCachePath('launch-library-2-v2.3.json');
  let cache = null;
...
export function tomtomProxy() {
  ...
  const CACHE_DIR = gevCachePath('tomtom');
...
export function adsbdbProxy() {
  ...
  const CACHE_PATH = gevCachePath('adsbdb.json');
...
/** Whether this process is a per-request serverless invocation. */
export function isServerlessRuntime() {
  return Boolean(process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME);
}

/**
 * Whether this host can own the AISStream websocket.
 *
 * The feed is a persistent socket plus a watchdog interval whose value is the
 * cache it accumulates over minutes; a serverless instance is killed between
 * requests, so connecting there spends the key's connection slot to deliver an
 * empty snapshot. GEV_AIS_STREAM forces the decision either way ('on' | 'off').
 *
 * @returns {boolean}
 */
export function aisStreamSupported() {
  const forced = String(process.env.GEV_AIS_STREAM || '').trim().toLowerCase();
  if (forced === 'off') return false;
  if (forced === 'on') return true;
  return !isServerlessRuntime();
}

/** The `/api/ais-live` body for a host that cannot hold the socket open. */
export function unsupportedAisPayload() {
  return {
    rows: [],
    source: 'AISStream',
    status: 'unsupported',
    error: 'live AIS needs a long-running server or AIS_LIVE_RELAY_URL',
    refreshing: false,
...
/**
 * Resolve the upstream URL for a relayed AIS request, or null when no relay is
 * configured. AIS_LIVE_RELAY_URL points at another host running this same
 * proxy (a long-running Node box), letting a serverless deployment keep the
 * layer live without holding the socket itself.
 *
 * @param {string} mountUrl - Request URL with the `/api/ais-live` mount stripped.
 * @returns {URL|null}
 */
export function aisRelayTarget(mountUrl) {
  const configured = String(process.env.AIS_LIVE_RELAY_URL || '').trim();
  if (!configured) return null;
  const base = new URL(configured);
  if (base.protocol !== 'https:' && base.protocol !== 'http:') return null;
  const sub = new URL(mountUrl || '/', 'http://localhost');
  const target = new URL(base.toString());
  target.pathname = `${base.pathname.replace(/\/+$/, '')}${sub.pathname === '/' ? '' : sub.pathname}`;
  target.search = sub.search;
  return target;
}

/**
 * Forward one AIS snapshot request to the configured relay.
 *
 * @param {URL} target - Relay URL.
 * @param {import('node:http').ServerResponse} res - Response to write.
 * @returns {Promise<void>}
 */
async function proxyAisRelay(target, res) {
  ...
  try {
    ...
    const body = await readResponseTextCapped(upstream, 8 * 1024 * 1024);
    ...
    res.end(JSON.stringify({
      ...
      status: 'down',
      error: `relay unreachable: ${error?.message || 'unknown error'}`,
    }));
  }
}
...
export function aisLiveProxy() {
  function install(middlewares) {
    middlewares.use('/api/ais-live', async (req, res) => {
      try {
        const relay = aisRelayTarget(req.url || '');
        if (relay) {
          await proxyAisRelay(relay, res);
          return;
        }
        if (!aisStreamSupported()) {
          res.statusCode = 503;
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.setHeader('Cache-Control', 'no-store');
          res.end(JSON.stringify(unsupportedAisPayload()));
          return;
        }
  ...
  return {
    ...
    configureServer(server) {
      install(server.middlewares);
      if (aisStreamSupported()) startAisStreamWatchdogTick();
...
const MILITARY_INSTALLATION_DISK_DIR = gevCachePath('military-installations');
/**
